...
def main() -> None:
    ...
    print(f"\nPipeline health check at {now.isoformat()}\n")
    ...
    if ingested.recent_count_24h == 0:
        print("  - No ingested articles in the last 24 hours.")
    if normalised.recent_count_24h == 0:
        print("  - No normalised articles in the last 24 hours.")
    if clustered_counts["clustered_24h"] == 0:
        print("  - No clustered articles in the last 24 hours.")
    if (
        ingested.recent_count_24h > 0
        and normalised.recent_count_24h > 0
        and clustered_counts["clustered_24h"] > 0
    ):
        print("  - Pipeline has processed fresh data in the last 24 hours.")
